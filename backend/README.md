...
│       └── analyst/           agent.py exports root_agent
├── deploy/
...
└── scripts/
    ├── ask_agent.py           run the agent locally
    └── ask_remote_agent.py    call the deployed agent
```
...
`landing` and `youtube` databases. The agent reaches it only through the
official `mcp-clickhouse` server (`list_databases`, `list_tables`,
`run_query`, ...). `services/clickhouse/mcp.py` picks the transport:
streamable HTTP when `STREAMLENS_MCP_URL` is set, otherwise a `uvx` stdio
subprocess.
