...
Three kinds of route:
    ...
    /api/dashboards     the dashboards the agent builds, and the data behind
                        each panel.
    /mcp/dashboards     the same dashboard tools spoken as MCP, for our agent
                        and for any other MCP client.
...
that stored query directly against ClickHouse.
"""
...
# Mounted below. Its session manager has to be started by a lifespan, and a
# mounted sub-app's own lifespan is never run, so the parent runs it.
...
async def lifespan(_: FastAPI):
    ...
    try:
        ...
        log.exception("could not ensure dashboard schema")
...
async def chat(body: ChatBody) -> StreamingResponse:
    """One turn with the curator, streamed as server-sent events."""
